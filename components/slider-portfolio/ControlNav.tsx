...
function ControlNav({
  ...
  useEffect(() => {
    setSwiper(
      restProps.parent?.current.querySelector('.bg-container > .swiper')?.swiper
    );
  ...
  return (
    <>
      <div
        className={dsnCN(
          'control-nav control-nav dsn-container p-absolute w-100 d-flex',
          className
        )}
        {...restProps}
      >
        ...
        <div className="slider-counter d-flex align-items-center">
          <span className="slider-current-index">
            {activeClass > 8 ? activeClass + 1 : `0${activeClass + 1}`}
          </span>
          <span className="slider-counter-delimiter" />
          <span className="slider-total-index">
