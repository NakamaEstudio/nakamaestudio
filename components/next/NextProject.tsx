...
function NextProject({ height, number, attrContent, heroContent, className }) {
  ...
  useEffect(() => {
    ...
    animate
      .fromTo(
        $('.bg img'),
        { y: -100, scale: 1, filter: 'blur(0px)' },
        { y: 0, scale: 1.2, filter: 'blur(10px)' }
      )
    ...
    const animate = gsap.timeline({
      scrollTrigger: {
        ...
        onLeave: ({ progress, direction, isActive }) =>
          router.push(getPortfolioLink(heroContent))

        // markers: true
      }
    });
    animate
      .to($('.case img'), { rotation: 360 }, 0)
      .to($('.bg img'), { scale: 1, filter: 'blur(0px)' }, 0)
      .to(
        $('.title-block.number'),
        { snap: { textContent: 1 }, textContent: '100%' },
        0
      )
  ...
  return (
    <>
      <div
        ref={ref}
        className={dsnCN(
          'next-project p-relative h-v-100 over-hidden',
          (src || video) && 'v-dark-head background-main',
          className
        )}
      >
        {/* @ts-ignore*/}
        ...
        >
          ...
          {category && (
            <MetaPost
              className="mt-30 p-absolute dsn-container right-0 top-0"
              category={category}
            />
          )}

          <Link
            href={getPortfolioLink(heroContent)}
          ...
          >
            <h2 className="title section-padding p-relative ">
              ...
              <span className="p-absolute section-padding d-none w-100 h-100 left-0 top-0">
                Cases de Sucesso
              </span>
            </h2>
          </Link>
          ...
          <div className="case">
            ...
            <strong className="title-block heading-color number v-middle">
              100%
            </strong>
          </div>
        </div>
